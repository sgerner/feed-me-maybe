...
const feed = db.prepare("SELECT id, title, custom_title FROM feeds LIMIT 1").get();
...
if (feed) {
  ...
  const feedAfter = db.prepare("SELECT id, title, custom_title FROM feeds WHERE id = ?").get(feed.id);
