...
export async function fetchFeed(url: string, options: { etag?: string, lastModified?: string } = {}): Promise<FetchResult> {
  try {
    // Validate URL
    const parsedUrl = new URL(url);
    if (!['http:', 'https:'].includes(parsedUrl.protocol)) {
      return { success: false, items: [], error: 'Only http/https URLs are supported' };
    }
    ...
    if (!response.ok) {
      return { success: false, items: [], error: `HTTP error ${response.status}`, httpStatus: response.status };
    }
