...
function extractFeatures(ctx: ArticleContext): Feature[] {
  ...
  try {
    ...
    for (const entity of aiEntities.slice(0, 5)) {
      ...
      if (token) {
        features.push({ type: 'entity', label: `entity:${token}`, weight: 1.0 });
      }
    }
  } catch {
  ...
  const contentType = normalizeToken(ctx.ai_content_type || '') || detectContentType(title, summary, content);
...
function getDecayedStrength(strength: number, lastReinforced: number): number {
  ...
  const ageDays = Math.max(0, (Date.now() - lastReinforced) / (24 * 60 * 60 * 1000));
...
function upsertPreference(type: string, label: string, polarity: 'positive' | 'negative', delta: number): void {
  ...
  ).get(type, label, polarity) as { id: string; strength: number; evidence_count: number; last_reinforced: number } | undefined;

  if (existing) {
    const decayed = getDecayedStrength(existing.strength || 0, existing.last_reinforced);
...
export function getPreferenceStateForArticle(articleId: string): PreferenceState {
  ...
  for (const row of rows) {
    ...
    if (!weight) continue;
    const decayedStrength = getDecayedStrength(row.strength || 0, row.last_reinforced || 0);
...
export function incorporateAiClassificationIntoMemory(articleId: string, classification: Partial<ArticleScore>): void {
  const topics = (classification.topics || []).map((t) => normalizeToken(String(t))).filter(Boolean).slice(0, 5);
  const entities = (classification.entities || []).map((e) => normalizeToken(String(e))).filter(Boolean).slice(0, 5);
  ...
  if (contentType) {
    upsertPreference('content_type', `content_type:${contentType}`, 'positive', 0.01);
  }

  // If AI identifies obituary-like content, pre-seed a weak negative filter.
  if (contentType === 'obituary' || topics.includes('obituary') || topics.includes('obituaries')) {
    upsertPreference('negative_filter', 'negative_filter:obituary', 'negative', 0.02);
  }
