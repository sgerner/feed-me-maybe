...
export function initializeDatabase(): void {
  ...
  db.prepare('CREATE INDEX IF NOT EXISTS idx_articles_feed_id ON articles(feed_id)').run();
  db.prepare('CREATE INDEX IF NOT EXISTS idx_articles_published_at ON articles(published_at DESC)').run();
  db.prepare('CREATE UNIQUE INDEX IF NOT EXISTS idx_articles_feed_url ON articles(feed_id, url)').run();
  db.prepare('CREATE INDEX IF NOT EXISTS idx_fetch_logs_feed_id ON feed_fetch_logs(feed_id)').run();
