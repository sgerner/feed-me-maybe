...
describe('settings API mutations', () => {
  ...
  it('deletes a specific preference via delete_pref_* key', async () => {
    ...
    const { applySettingsMutation } = await import('./+server');
    const db = getDb();
    const now = Date.now();
    ...
    ).run('pref-1', 'topic:obituary', 'topic', 'negative', 0.7, 2, now, '', now);

    const result = applySettingsMutation('delete_pref_pref-1', 'true');
    const row = db.prepare('SELECT id FROM user_preference_memory WHERE id = ?').get('pref-1');
  ...
  it('resets training data and article interaction state', async () => {
    ...
    ).run('article-1', 'feed-1', 'https://example.com/a1', 'A1', 1, 1, 1, 1, 0, 92, 88, now, now, now);
    db.prepare(
    ...
    ).run('pref-2', 'topic:obituary', 'topic', 'negative', 0.9, 5, now, '', now);
    db.prepare(
    ...
    const interactions = db.prepare('SELECT COUNT(*) as c FROM user_interactions').get() as { c: number };
    const prefs = db.prepare('SELECT COUNT(*) as c FROM user_preference_memory').get() as { c: number };
    const aiMeta = db.prepare('SELECT COUNT(*) as c FROM article_ai_metadata').get() as { c: number };
    const article = db.prepare('SELECT hidden, read, saved, thumbs_up, thumbs_down, heuristic_score, combined_score FROM articles WHERE id = ?').get('article-1') as Record<string, number>;
