...
<div class="container mx-auto max-w-lg px-4 py-10">
  ...
  <div class="mb-10 flex items-center justify-center gap-3">
    {#each [1, 2, 3, 4] as s (s)}
      <div
        ...
        class:step-dot-inactive={s > step}
      ></div>
    {/each}
  </div>
  ...
  {#if step === 1}
    <div class="glass-card p-8 text-center" in:fly={{ y: 16, duration: 350 }}>
      <div
        class="mb-6 inline-flex h-20 w-20 items-center justify-center rounded-full"
        style="background: color-mix(in oklch, var(--color-primary-500) 12%, transparent); box-shadow: 0 0 30px -5px color-mix(in oklch, var(--color-primary-500) 30%, transparent);"
      >
        <svg
      ...
      <button
        class="btn preset-filled-primary-500 mt-8 inline-flex items-center gap-2"
        onclick={nextStep}
      >
        Get Started
        <svg
  ...
  {:else if step === 2}
    <div class="glass-card p-6 md:p-8" in:fly={{ y: 16, duration: 350 }}>
      <h2 class="text-xl font-bold" style="color: var(--color-surface-50);">
        Add Your First Feed
      </h2>
      <p
        class="mt-2 text-sm"
  ...
  {:else if step === 4}
    <div class="glass-card p-8 text-center" in:fly={{ y: 16, duration: 350 }}>
      <div
        class="mb-6 inline-flex h-20 w-20 items-center justify-center rounded-full"
        style="background: color-mix(in oklch, var(--color-success-500) 12%, transparent); box-shadow: 0 0 30px -5px color-mix(in oklch, var(--color-success-500) 30%, transparent);"
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="40"
          ...
          stroke="currentColor"
          stroke-width="1.5"
          style="color: var(--color-success-400);"
        >
          <path d="M20 6 9 17l-5-5" />
        </svg>
      </div>
      <h2 class="text-2xl font-bold" style="color: var(--color-surface-50);">
        You're All Set!
      </h2>
      <p
        class="mt-4"
        style="color: color-mix(in oklch, var(--color-surface-200) 60%, transparent);"
      >
        Start reading your feeds. You can add more feeds anytime from the Feeds
        page.
      </p>
      <button
        class="btn preset-filled-primary-500 mt-8 inline-flex items-center gap-2"
