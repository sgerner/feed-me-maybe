  ...
  {:else}
    <div class="grid grid-cols-1 gap-3 lg:grid-cols-2">
      {#each pageData.articles as article, i (article.id)}
        <div class="glass-card glass-card-hover flex items-start justify-between p-4 md:p-5" in:fly={{ y: 14, duration: 320, delay: Math.min(i * 35, 350) }}>
          <div class="min-w-0 flex-1">
            <span class="mb-1 inline-flex items-center px-2 py-0.5 text-xs font-medium" style="background: color-mix(in oklch, var(--color-primary-500) 10%, transparent); color: var(--color-primary-300); border-radius: 2px;">{article.feed_title}</span>
