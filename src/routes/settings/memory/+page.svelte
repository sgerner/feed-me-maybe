...
{#if msg}
  <div class="mt-5 flex items-center gap-2 px-3 py-2 text-sm" style="background: color-mix(in oklch, var(--color-success-500) 10%, transparent); color: var(--color-success-300); border: 1px solid color-mix(in oklch, var(--color-success-500) 20%, transparent); border-radius: 2px;" in:fly={{ y: 6, duration: 220 }}>
      ...
      <div class="glass-card glass-card-hover p-4" in:fly={{ y: 10, duration: 280, delay: Math.min(i * 30, 300) }}>
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-2">
            <span class="inline-flex items-center px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider" style={p.polarity === 'positive' ? 'background: color-mix(in oklch, var(--color-success-500) 12%, transparent); color: var(--color-success-300); border-radius: 2px;' : 'background: color-mix(in oklch, var(--color-warning-500) 12%, transparent); color: var(--color-warning-300); border-radius: 2px;'}>
              {p.type}
            </span>
            <span class="text-sm font-medium" style="color: var(--color-surface-50);">{p.label}</span>
          </div>
          <button class="flex h-7 w-7 items-center justify-center text-xs transition-colors hover:bg-surface-100/10" style="color: color-mix(in oklch, var(--color-surface-200) 45%, transparent); border-radius: 2px;" aria-label="Delete preference" onclick={() => deletePref(p.id)}>
        ...
        <div class="mt-2 flex items-center gap-3 text-xs" style="color: color-mix(in oklch, var(--color-surface-200) 45%, transparent);">
          <div class="h-2 w-28 overflow-hidden" style="background: color-mix(in oklch, var(--color-surface-100) 8%, transparent); border-radius: 2px;">
            <div class="h-2 transition-all duration-500" style="width: {p.strength * 100}%; background: linear-gradient(90deg, var(--color-primary-500), var(--color-secondary-500)); box-shadow: 0 0 8px color-mix(in oklch, var(--color-primary-500) 40%, transparent); border-radius: 2px;"></div>
          </div>
