  ...
  async function importOpml() {
    ...
    input.onchange = async () => {
      ...
      if (res.ok) {
        ...
        alert(`Imported ${result.imported} feeds. ${result.errors?.length || 0} errors.`);
...
<div class="space-y-5">
  ...
  <div class="glass-card glass-card-hover p-5" in:fly={{ y: 12, duration: 300, delay: 60 }}>
    ...
    <form method="POST" action="?/updatePolling" use:enhance class="flex items-end gap-4">
      <label class="label flex-1">
        <span class="mb-1 block text-xs" style="color: color-mix(in oklch, var(--color-surface-200) 55%, transparent);">Global Poll Interval (minutes)</span>
        <input
          name="interval"
          type="number"
          min="1"
    ...
    <p class="mt-3 text-xs" style="color: color-mix(in oklch, var(--color-surface-200) 45%, transparent);">
      Feeds are polled every {data.pollInterval} minutes. Adaptive polling is enabled: feeds that haven't changed in a while will be polled less frequently to save bandwidth.
    </p>
  </div>
  ...
  <div class="glass-card glass-card-hover p-5" in:fly={{ y: 12, duration: 300, delay: 240 }}>
    ...
    <p class="mb-4 text-xs" style="color: color-mix(in oklch, var(--color-surface-200) 45%, transparent);">Import or export your feeds as OPML.</p>
    <div class="flex gap-2">
      ...
      <a href="/api/opml/export" class="btn preset-filled-primary-500 inline-flex items-center gap-2 text-xs no-underline">
