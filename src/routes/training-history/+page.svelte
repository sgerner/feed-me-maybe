...
<div class="mx-auto max-w-2xl">
  ...
  {:else}
    <div class="space-y-3">
      {#each pageData.articles as a (a.id)}
        <div class="glass-card glass-card-hover flex items-center justify-between p-4">
          <div class="min-w-0 flex-1">
            <span class="mb-1 inline-flex items-center rounded-md px-2 py-0.5 text-xs font-medium" style="background: color-mix(in oklch, var(--color-primary-500) 10%, transparent); color: var(--color-primary-300);">{a.feed_title}</span>
